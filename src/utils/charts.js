export class ChartUtils {
  ...
  generateLinearDataSource(xAxisValueType, yAxisValueType) {
    let array = [[yAxisValueType]];
    for(let i=0; i < this.data.hits.hits.length; i++) {
      console.log(this.data.hits.hits[i]._source[yAxisValueType]);
      // array[0].push(this.data.hits.hits[i]._source[yAxisValueType])
    //   for(let j=0; j < this.data.aggregations.transaction.buckets[0].percentiles.values.length; j++) {
    //     if(i === 0) {
    //       array.push([]);
    //       array[j+1].push(this.data.aggregations.transaction.buckets[0].percentiles.values[j].key);
